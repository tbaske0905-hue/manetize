...
""",
    ...
    "lp": {
        "label": "LP・ホームページ コーディング",
        "template": """
はじめまして。HTML/CSS/JavaScriptを使ったLP・Webサイト制作を行っているエンジニアです。

【ご質問への回答】
■ 使用CSSネーミングルール：BEM記法を使用しています。SCSSは使用しません。
■ LP1ページあたりの概算見積：{price_per_page}（デザインカンプ提供前提）
■ 標準納期：{days_per_page}（カンプの複雑さによって前後します）
■ 稼働状況：平日日中連絡可能・即着手可能な状態です
■ 6月15日納品：対応可能です
■ 類似LP制作実績：
  https://tbaske0905.netlify.app/templates/lp_corporate/
  （コーポレート向けLPのサンプルです。レスポンシブ対応・PC/SP確認済み）

{custom_point}

デザインカンプをご共有いただければ、より具体的な見積もり・スケジュールをお伝えできます。
どうぞよろしくお願いいたします。
""",
        "hints": {
            "price_per_page": "15,000〜25,000円/ページ",
            "days_per_page": "2〜3営業日/ページ",
            "custom_point": "案件固有の一言（継続案件への意欲・対応実績など）",
        },
    },
    "data": {
...
if __name__ == "__main__":
    ...
    group.add_argument("--category", choices=list(TEMPLATES.keys()), help="案件カテゴリ（vba/scraping/automation/data/lp）")
